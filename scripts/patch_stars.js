...
const newMaterialCode = `
            ...
            const material = new THREE.MeshPhysicalMaterial({
                ...
                emissive: 0x222222,    // 약간의 발광으로 그라데이션 어두운 부분 보정
                roughness: 0.2,        // 매끄러운 금속
...
const newLightCode = `
            // 은색 메테리얼에 그라데이션 느낌을 주기 위한 다채로운 조명 세팅
            const pointLight1 = new THREE.PointLight(0x3b82f6, 2.0, 100); // 파란빛
            pointLight1.position.set(15, 15, 10);
            scene.add(pointLight1);

            const pointLight2 = new THREE.PointLight(0x8b5cf6, 2.0, 100); // 보랏빛
...
html = html.replace(/radial-gradient\(ellipse at 50% -20%, rgba\(30, 58, 138, 0\.4\), transparent 60%\), \n                radial-gradient\(circle at bottom right, rgba\(15, 118, 110, 0\.2\), transparent 50%\),\n                linear-gradient\(to bottom, transparent, rgba\(0, 0, 0, 0\.4\)\);/g, `radial-gradient(ellipse at 50% 50%, rgba(15, 23, 42, 1), transparent 80%), 
                ...
                radial-gradient(circle at 80% 20%, rgba(76, 29, 149, 0.4), transparent 50%),
