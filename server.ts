...
function generateFallbackAnswer(userQuestion: string, companyName: string, sectionTitle: string, answer: string) {
  ...
  if (q.includes("뭐써") || q.includes("뭐 써") || q.includes("소재") || q.includes("아이디어") || q.includes("모르겠") || q.includes("쓸게") || q.includes("주제")) {
    return "거창한 성과가 아니어도 괜찮습니다. 전공 실습 시간의 제작 과제, 팀 프로젝트에서 의견을 조율했던 일, 동아리 활동, 또는 기능사 자격증을 준비하며 부족한 부분을 연습했던 일 중 하나를 떠올려보세요. 작은 문제 상황에서 시작해 본인의 변화를 적는 것이 가장 좋은 소재가 됩니다.";
  }
  if (q.includes("어때") || q.includes("어떠") || q.includes("평가") || q.includes("봐줘")) {
    const len = answer ? answer.trim().length : 0;
    if (len < 30) {
      return "아직 작성된 내용이 짧아 본격적인 진단이 어렵습니다. 우선 떠오르는 생각대로 편하게 2~3문장 정도 적어주시면, 문맥과 구성 요소를 분석해 실시간으로 보완점을 짚어드리겠습니다.";
    }
    return "현재 작성 중인 흐름은 좋습니다. 문장이 너무 길어지면 핵심이 흐려질 수 있으니, 한 문장에 하나의 생각만 담아 간결하게 끊어주시고 본인이 직접 주도한 구체적인 행동(Action)을 한 번 더 강조해보세요.";
  }
  ...
  if (q.includes("어색") || q.includes("자연스러") || q.includes("맞춤법")) {
    return "전체 문맥이 진솔하고 명확합니다. 문장이 너무 길어지지 않게 한 문장에 하나의 내용만 담고, '~인 것 같습니다' 대신 '~했습니다'로 단정형 종결어미를 쓰면 훨씬 자신감 있게 느껴집니다.";
  }
